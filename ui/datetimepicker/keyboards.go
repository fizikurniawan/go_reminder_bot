...
func (datePicker *DatePicker) buildHourKeyboard() [][]models.InlineKeyboardButton {
	var keyboard [][]models.InlineKeyboardButton

	var row1 []models.InlineKeyboardButton
	var row2 []models.InlineKeyboardButton
	for i := 1; i < 7; i++ {
		hour := strconv.Itoa(i)
		hour2 := strconv.Itoa(i + 6)
		row1 = append(row1, models.InlineKeyboardButton{Text: hour, CallbackData: datePicker.encodeState(state{cmd: cmdHourClick, param: i})})
		row2 = append(row2, models.InlineKeyboardButton{Text: hour2, CallbackData: datePicker.encodeState(state{cmd: cmdHourClick, param: i + 6})})
	}
...
func (datePicker *DatePicker) buildMinuteKeyboard() [][]models.InlineKeyboardButton {
	var keyboard [][]models.InlineKeyboardButton

	// Define the number of rows and buttons per row
	numRows := 10
	buttonsPerRow := 6
	...
	for row := 0; row < numRows; row++ {
		...
		// Calculate the starting minute for the row
		startMinute := row * buttonsPerRow

		for i := 1; i <= buttonsPerRow; i++ {
			if row == numRows-1 {
				if i == 6 {
					continue
				}
			}
			minute := strconv.Itoa(startMinute + i)
			keyboardRow = append(keyboardRow, models.InlineKeyboardButton{Text: minute, CallbackData: datePicker.encodeState(state{cmd: cmdMinuteClick, param: startMinute + i})})
		}
...
func (datePicker *DatePicker) buildPeriodeKeyboard() [][]models.InlineKeyboardButton {
	var keyboard [][]models.InlineKeyboardButton

	var row []models.InlineKeyboardButton
	row = append(row, models.InlineKeyboardButton{Text: "AM", CallbackData: datePicker.encodeState(state{cmd: cmdPeriodeClick, param: 0})})
	row = append(row, models.InlineKeyboardButton{Text: "PM", CallbackData: datePicker.encodeState(state{cmd: cmdPeriodeClick, param: 12})})
